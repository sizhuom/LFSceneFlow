...
%         ope.lambda_q = 3;
    ...
    case {'clg2d-q'}
        ...
        sig = 1e-3;
        for i = 1:length(ope.spatial_filters);
            ope.rho_spatial_u{i}   = robust_function(method, sig);
            ope.rho_spatial_v{i}   = robust_function(method, sig);
            ope.rho_spatial_w{i}   = robust_function(method, sig);
        end;
        ope.rho_data        = robust_function(method, sig);
        ope.display  = true;      
    ...
    case {'tv-l1'}
        ope = tv_l1_scene_flow;
        ope.display = true;
        
    case {'classic+nl-xy'}
        ope = classic_nl_xy_scene_flow;
    ...
    case {'hs-brightness-xy'}
        % brightness constancy, with median filtering
        ope = hs_xy_scene_flow;
        ope.median_filter_size   = median_filter_size;
        ope.lambda   = 10;
        ope.lambda_q = 10;    
        
    case {'classic+nl-z'}
        ope = classic_nl_z_scene_flow;
