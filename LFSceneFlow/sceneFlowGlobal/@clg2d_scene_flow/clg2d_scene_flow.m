function this = clg2d_scene_flow(varargin)
%
...
narginchk(0, 1);
  ...
  switch (length(varargin))
    case 0
      ...
      this.lambdaZ         = 1;    % weight for z motion
      this.lambdaZ_q       = 1;
      ...
      this.limit_update    = false;      % limit the flow incrment to be less than 1 per linearization step
      this.display         = false;            
      
      this.solver          = 'sor';   % 'sor' 'pcg' for machines with limited moemory       
      ...
      this.interpolation_method = 'bi-linear';      %'cubic', 'bi-linear', 'bi-cubic'
      ...
      for i = 1:length(this.spatial_filters);
          ...
          this.rho_spatial_w{i}   = robust_function(method, sig, a);
      end;
      ...
      this.sigmaP           = [0 0 0 0]; % prefiltering
      
      this.alphaCen = [];
      this.alphaMap = []; % alpha for the EPI structures
      
      this.owSize = [6 6 1 1];
      this.owSigma = [6 6 1 1];
      this.owOccSigma = 0.3;
      
      % smoothness term weight based on precomputed xyflow
      this.weightMap = [];
      this.xyflow = []; % used for segmentation
      this.xywSigma = 0.3;
      this.xywr = 3;
      
      % smoothness term weight based on alpha map
      this.awSigma = 0.6;
      this.awr = 3;
      
      % zero-motion term weight
      this.mu = 0;
      ...
      this = class(this, 'clg2d_scene_flow');         
      
    case 1
      if isa(varargin{1}, 'clg2d_scene_flow')
        this = varargin{1};        
      else    
          this = clg2d_scene_flow;
