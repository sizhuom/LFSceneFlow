...
for i = 1:this.max_warping_iters
    ...
    while (error > this.epsilon^2 && n < this.max_iters)
        ...
        d1(mask1) = l_t * IX(mask1);
        d2(mask1) = l_t * IY(mask1);
        d3(mask1) = l_t * IZ(mask1);
        d1(mask2) = -l_t * IX(mask2);
        d2(mask2) = -l_t * IY(mask2);
        d3(mask2) = -l_t * IZ(mask2);
        fi = -rho ./ grad;
        ...
        uk = u;
        u = v + this.theta * cat(5, div_p1, div_p2, div_p3);
        error = u - uk;
        error = sum(error(:).^2);
        ...
        ng2 = 1 + taut * g2;
        ng3 = 1 + taut * g3;
%         p1(:,:,:,:,1) = (p1(:,:,:,:,1) + taut * u1x) ./ ng1;
%         p1(:,:,:,:,2) = (p1(:,:,:,:,2) + taut * u1y) ./ ng1;
%         p1(:,:,:,:,3) = (p1(:,:,:,:,3) + taut * u1u) ./ ng1;
%         p1(:,:,:,:,4) = (p1(:,:,:,:,4) + taut * u1v) ./ ng1;
%         p2(:,:,:,:,1) = (p2(:,:,:,:,1) + taut * u2x) ./ ng2;
%         p2(:,:,:,:,2) = (p2(:,:,:,:,2) + taut * u2y) ./ ng2;
%         p2(:,:,:,:,3) = (p2(:,:,:,:,3) + taut * u2u) ./ ng2;
%         p2(:,:,:,:,4) = (p2(:,:,:,:,4) + taut * u2v) ./ ng2;
        p1 = bsxfun(@rdivide, p1 + taut * cat(5,u1x,u1y,u1u,u1v), ng1);
        p2 = bsxfun(@rdivide, p2 + taut * cat(5,u2x,u2y,u2u,u2v), ng2);
        p3 = bsxfun(@rdivide, p3 + taut * cat(5,u3x,u3y,u3u,u3v), ng3);
    end
