function this = tv_l1_scene_flow(varargin)
%
...
narginchk(0, 1);
  ...
  switch (length(varargin))
    case 0
      ...
      this.max_warping_iters    = 5;           % # of warping/linearization per pyramid level
      ...
      this.deriv_filter         = [-1 0 1]/2;   %[1 -8 0 8 -1]/12;
      this.display              = false;
      ...
      % placeholders
      this.tau = 0.125; % time step
      this.theta = 0.04; %0.3; % weight parameter for (u-v)^2
      this.epsilon = 0.01; % tolerance for numerical convergence
      this.max_iters = 300; % max iteration for every warp
      ...
      this = class(this, 'tv_l1_scene_flow');         

      
    case 1
      if isa(varargin{1}, 'tv_l1_scene_flow')
        this = other;
        
      else    
          this = tv_l1_scene_flow;
      end
