function opqo = denoise_color_weighted_medfilt2(opq, im, occ, bfhsz, mfsz, sigma_i, fullVersion)
...
if nargin < 5
    ...
    opqo(:,:,1) = medfilt2(opq(:,:,1), mfsz, 'symmetric');
    opqo(:,:,2) = medfilt2(opq(:,:,2), mfsz, 'symmetric');
    opqo(:,:,3) = medfilt2(opq(:,:,3), mfsz, 'symmetric');
end;
...
pad_o  = padarray(opq(:,:,1), bfhsz*[1 1], 'symmetric', 'both');        
pad_p  = padarray(opq(:,:,2), bfhsz*[1 1], 'symmetric', 'both');        
pad_q  = padarray(opq(:,:,3), bfhsz*[1 1], 'symmetric', 'both');
...
for ib = 1:nB;
    ...
    neighbors_o = pad_o(pad_indx);
    neighbors_p = pad_p(pad_indx);
    neighbors_q = pad_q(pad_indx);

    % solve weighted median filtering
    indx = sub2ind(sz, indx_row, indx_col);
    oo   = opqo(:,:,1);
    o    = weighted_median(weights, neighbors_o);
%     u    = weighted_median_iter(weights, neighbors_u);
    oo(indx) = o;
    po   = opqo(:,:,2);
    p    = weighted_median(weights, neighbors_p);
%     v    = weighted_median_iter(weights, neighbors_v);
    po(indx) = p;
    qo   = opqo(:,:,3);
    q    = weighted_median(weights, neighbors_q);
%     u    = weighted_median_iter(weights, neighbors_u);
    qo(indx) = q;
    opqo = cat(3, oo, po, qo);
