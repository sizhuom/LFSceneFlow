...
% M = sparse(rows, cols, vals, prod(sizeF + sz - 1), nblks);

% Sizhuo: break into blocks to handle large light fields
% MAX_ROW = 2500000;
timerVal = tic();
MAX_ROW = 1000000;
MBlocks = ceil(nblks / MAX_ROW);
M2 = sparse(prod(sizeF + sz - 1), nblks);
for i = 1:MBlocks
    if i < MBlocks
        rows = reshape(repmat(imgpos(1+MAX_ROW*(i-1):MAX_ROW*i)', blksz, 1), MAX_ROW*blksz, 1) + ...
            repmat(fltpos - 1, MAX_ROW, 1);
        cols = reshape(repmat(1+MAX_ROW*(i-1):MAX_ROW*i, blksz, 1), MAX_ROW*blksz, 1);
        vals = repmat(F(:), MAX_ROW, 1);
    else
        leftover = nblks - MAX_ROW * (MBlocks - 1);
        rows = reshape(repmat(imgpos(1+MAX_ROW*(i-1):nblks)', blksz, 1), leftover*blksz, 1) + ...
            repmat(fltpos - 1, leftover, 1);
        cols = reshape(repmat(1+MAX_ROW*(i-1):nblks, blksz, 1), leftover*blksz, 1);
        vals = repmat(F(:), leftover, 1);
    end
    ...
    % Build sparse output array
    M2 = M2 + sparse(rows, cols, vals, prod(sizeF + sz - 1), nblks);
end
timeElapsed = toc(timerVal);
% fprintf('convmtxn: %f seconds\n', timeElapsed);
M = M2;