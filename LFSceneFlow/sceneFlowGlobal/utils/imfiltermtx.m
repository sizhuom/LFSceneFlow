...
% THIS SOFTWARE.        
%
% Sizhuo: modified to make it really work for N-D!
  

  % Sizhuo: adapted to the case where length(size(F)) < length(sz)
  Fsize = ones(1,length(sz));
  Fsize(1:length(size(F))) = size(F);
  % Size of padding for boundary handling 
  pad_size = 0.5 * (Fsize - 1);
  
  % Corners of the original image in the padded image
  corner_lo = 1 + pad_size;
  corner_hi = Fsize + sz - 1 - pad_size;
  

  % Pixel index of the output image M
  M_idx = zeros(sz + Fsize - 1);
  M_idx(:) = 1:numel(M_idx);
...
%            zeros(pad_size(1), sz(2) + size(F, 2) - 1)];
  ndims = numel(Fsize);
  for i = 1:ndims
      pad_fullsize = size(X_idx);
      pad_fullsize(i) = pad_size(i);
      X_idx = cat(i, zeros(pad_fullsize), X_idx, zeros(pad_fullsize));
  end
...
%       end
      ...
      for i = 1:ndims
          ...
          edge_hi = expand_hi;
          edge_lo(i) = 1;
          edge_hi(i) = expand_lo(i)-1;
          rep_dim = ones(1, ndims);
          rep_dim(i) = edge_hi(i) - edge_lo(i) + 1;
          command = 'M_idx(';
          for j = 1:ndims
              command = [command sprintf('%d:%d',edge_lo(j),edge_hi(j))];
              if j ~= ndims
                  command = [command ','];
              end
          end
          command = [command ')=repmat(M_idx('];
          for j = 1:ndims
              if j == i
                  command = [command sprintf('%d',expand_lo(i))];
              else
                  command = [command sprintf('%d:%d',expand_lo(j),expand_hi(j))];
              end
              if j ~= ndims
                  command = [command ','];
              end
          end
          command = [command '),[' sprintf('%d ',rep_dim) ']);'];
          eval(command);
          
          edge_lo(i) = expand_hi(i)+1;
          edge_hi(i) = size(M_idx, i);
          rep_dim = ones(1, ndims);
          rep_dim(i) = edge_hi(i) - edge_lo(i) + 1;
          command = 'M_idx(';
          for j = 1:ndims
          ...
          for j = 1:ndims
              if j == i
                  command = [command sprintf('%d',expand_hi(i))];
              else
                  command = [command sprintf('%d:%d',expand_lo(j),expand_hi(j))];
              end
              if j ~= ndims
...
%                       repmat(c(:)', length(r), 1));
  ...
  M = make_convn_mat(F, sz + Fsize - 1, 'same');
  [rows, cols, vals] = find(M);

  % Re-assemble the sparse array, but use the column map to accumulate
  % rows appropriately
  M = sparse(rows, col_map(cols), vals, prod(Fsize + sz - 1), prod(sz));
