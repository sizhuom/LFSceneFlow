function q = compute_flow_base(this, q)
%%
...
for i = 1:this.max_iters
    ...
    [It, ~, ~, IZ] = partial_deriv(this.images, this.intr, cat(5,this.xyflow,q), this.interpolation_method,...
...
%         evaluate_energy_map(this,opq);
        ...
        if this.alpha == 1
            [A, b, parm, iterative] = ...
                flow_operator(qua_this, q, dq, It, IZ);
            
        elseif this.alpha > 0
            [A, b] = ...
                flow_operator(qua_this, q, dq, It, IZ);
            [A1, b1, parm, iterative] = ...
                flow_operator(this, q, dq, It, IZ);
...
%                 [x, flag, res, n] = sor(A', b, 1.9, this.sor_max_iters, 1E-2, opq(:));
            ...
            case 'bicgstab'
                [x,flag] = reshape(bicgstab(A, b, 1E-3, 200, [], [], q(:)), size(q)); %, parm
            case 'pcg'
...
%             occ = detect_occlusion(q, this.images, this.intr);
%             for mi = 1:size(q,1)
%                 for mj = 1:size(q,2)
%                     q(mi,mj,:,:,:) = denoise_color_weighted_medfilt3_2d(...
%                         squeeze(q(mi,mj,:,:,:)), ...
...
%         end;
        ...
        if ~isempty(this.median_filter_size)
            LFMedfilt2(q, this.median_filter_size);
        end;
