function L = evaluate_log_posterior(this, q)
...
%     q_ = convn(opq(:,:,:,:,3), S{i}, 'valid');   
    ...
    if isempty(this.weightMap)
        if isa(this.rho_spatial_u{i}, 'robust_function')
            ...
            p = p - this.lambdaZ*sum(evaluate(this.rho_spatial_w{i}, q_(:)));
            
        elseif isa(this.rho_spatial_u{i}, 'gsm_density')
            
            p   = p + this.lambdaZ*sum(evaluate_log(this.rho_spatial_w{i}, q_(:)'));
        ...
        if isa(this.rho_spatial_u{i}, 'robust_function')
            ...
            p = p - this.lambdaZ*sum(w.*evaluate(this.rho_spatial_w{i}, q_(:)));
            
        elseif isa(this.rho_spatial_u{i}, 'gsm_density')
            
            p   = p + this.lambdaZ*sum(w.*evaluate_log(this.rho_spatial_w{i}, q_(:)'));
...
% likelihood
It = partial_deriv(this.images, this.intr, cat(5,this.xyflow,q), this.interpolation_method,...
